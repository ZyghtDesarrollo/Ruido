...
+ function(a) {
    ...
    var d = function(c, e) {
        ...
        for (var f in e.custom)
            if (!e.errors[f])
                throw new Error("Missing default error message for custom validator: " + f);
    ...
    d.INPUT_SELECTOR = ':input:not([type="submit"], button):enabled:visible', d.FOCUS_OFFSET = 20, d.DEFAULTS = {
        ...
        var e = b(c), f = c.data("bs.validator.previous"), g = c.data("bs.validator.errors");
        if (f === e)
        ...
        var f = [], g = a.Deferred(), h = this.options;
